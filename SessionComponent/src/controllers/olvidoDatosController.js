...
class olvidoDatosController{
    static getOlvidoDatos = async (req, res) => { 
        if(iSession.sessionExist(req)) return res.json({message: "Tienes una sesion activa, debes desloguearte para poder recuperar los datos"})

        return res.json({message: "Estas en el GET para recuperar los datos, envia el username a recuperar con el metodoo POST"})
    }
    ...
    static getCargarPreguntas = async (req, res) => { 
        if (!req.session.questions) return res.json({ error: "No estas solicitando recuperar los datos, por lo que no puedes acceder a esta ruta" })
